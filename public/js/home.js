...
async function loadTopics() {
    try {
        ...
        if (!token) {
            throw new Error(
                "No se encontró un token. Inicia sesión para continuar."
            );
        }
        ...
        if (!response.ok) {
            throw new Error(
                "Error al cargar los temas: " + response.statusText
            );
        }
        ...
        topics.forEach((topic, index) => {
            ...
            // Verificar si el nombre del tema es "Minijuegos"
            let redirectURL =
                topic.name.toLowerCase() === "minijuegos"
                    ? "/listgames" // Redirigir a otra vista si es "Minijuegos"
                    : `level_preview/${topic.id}`; // Redirigir normalmente para otros temas
            ...
            item.innerHTML = `
                ...
                <div class="buttons-container">
                    <a href="${redirectURL}"><button class="button2">JUGAR</button></a>
                </div>
            `;
        ...
        alert(
            "Hubo un problema al cargar los temas. Por favor, inténtalo más tarde."
        );
    }
}
